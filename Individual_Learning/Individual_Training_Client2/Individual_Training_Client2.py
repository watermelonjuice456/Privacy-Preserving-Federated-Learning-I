...
def train_model(epochs):
    train_data = pd.read_csv('../../train_validation_test_minmax/train_client2.csv')
    train_data = train_data.sample(frac = 1)
    print('reading train_data')
    print(train_data)

    validation_data = pd.read_csv('../../train_validation_test_minmax/validation_client2.csv')
    ...
    #os.mkdir('client2_model')
    checkpoint = ModelCheckpoint(filepath='client2_model/model_{epoch:04d}.h5', period = 1)
    ...
    np.save('history_epoch_{}_client2.npy'.format(epochs), history.history)
...
def evaluate_model(virus, Path, threshold):
    model = keras.models.load_model(Path)
    #tr = 0.022392549562451312
    tr = threshold

    if not path.exists('logbook_client2.csv'):
        with open('logbook_client2.csv', 'w', newline='') as logbook:
    ...
    if virus == 'BASHLITE':
        ...
        test_data = pd.read_csv('../../train_validation_test_minmax/test_BASHLITE_client2.csv')
    elif virus == 'mirai':
        devices = ['Ecobee_Thermostat', 'Provision_PT_737E_Security_Camera', 'Philips_B120N10_Baby_Monitor',
                   'Provision_PT_838_Security_Camera', 'SimpleHome_XCS7_1002_WHT_Security_Camera',
                   'Danmini_Doorbell',
                   'SimpleHome_XCS7_1003_WHT_Security_Camera']
        test_data = pd.read_csv('../../train_validation_test_minmax/test_mirai_client2.csv')
    ...
    with open('logbook_client2.csv', 'a', newline='') as logbook:
        writer = csv.writer(logbook)
