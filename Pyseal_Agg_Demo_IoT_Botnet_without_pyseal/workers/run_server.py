...
if __name__ == "__main__":
    ...
    app3 = create_app(worker_id=3, config_object=config)
    app_server1 = WSGIServer((WORKER_IP, 7101), app1)
    app_server2 = WSGIServer((WORKER_IP, 7102), app2)
    app_server3 = WSGIServer((WORKER_IP, 7103), app3)
    monkey.patch_thread()
    t1 = threading.Thread(target=app_server1.serve_forever)
    t2 = threading.Thread(target=app_server2.serve_forever)

    t1.setDaemon(True)
    t2.setDaemon(True)
    t1.start()
    t2.start()
    app_server3.serve_forever()
