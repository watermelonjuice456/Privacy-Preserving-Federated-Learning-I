...
def train_model(epochs):
    ...
    #os.mkdir('centralized_model_batch_size_1024_minmax')
    checkpoint = ModelCheckpoint(filepath='centralized_model_batch_size_1024_minmax/model_{epoch:04d}.h5', period = 1)
...
def evaluate_model(virus, Path, threshold):
    ...
    if not path.exists('logbook_centralized.csv'):
        with open('logbook_centralized_accuracy.csv', 'w', newline='') as logbook:
    ...
    if virus == 'BASHLITE':
        ...
        test_data = pd.read_csv('../train_validation_test_minmax/test_BASHLITE_separate_minmax.csv')
    elif virus == 'mirai':
        devices = ['Ecobee_Thermostat', 'Provision_PT_737E_Security_Camera', 'Philips_B120N10_Baby_Monitor',
                   'Provision_PT_838_Security_Camera', 'SimpleHome_XCS7_1002_WHT_Security_Camera',
                   'Danmini_Doorbell',
